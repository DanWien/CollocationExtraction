    ...
    public static class MapperClass extends Mapper<LongWritable, Text, Text, Text> {
        ...
        public void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
            ...
            if (parts.length >= 5) {
                ...
                if (words.length == 2) {
                    context.write(new Text(decade + "\t" + words[1] + "\t*"), new Text(count));
                }
            }
        }
    }
        ...
        public void reduce(Text key, Iterable<Text> values, Context context) throws IOException, InterruptedException {
            ...
            if(keyParts.length == 3) {
                ...
                for(Text val:values) {
                    ...
                    String bigram = keyParts[1];
                    String den = valParts[2];
                    double npmi = partialPMI / Double.parseDouble(den);
                    String[] words = bigram.split("\\s+");
                    bigram = words[1] + " " + words[0];
                    context.write(new Text(decade + "\t" + bigram),new Text(String.valueOf(npmi)));
                }
            }
        }
    }
...
//        job.setCombinerClass(ReducerClass.class);
        ...
        FileInputFormat.addInputPath(job, new Path("s3://this-is-my-first-bucket-dn2/step2_output"));
        FileOutputFormat.setOutputPath(job, new Path("s3://this-is-my-first-bucket-dn2/step3_output"));
